...
export default function Services() {
  ...
  // Datos enriquecidos con métricas y herramientas para llenar los vacíos visuales
  const services = [
    {
      id: 'security',
  ...
  return (
    <div className="pt-32 min-h-screen bg-[#f8fafc] dark:bg-[#050d0a] relative overflow-hidden transition-colors duration-500">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute bottom-[40%] right-[-10%] w-[800px] h-[800px] rounded-full bg-emerald-400/10 dark:bg-emerald-500/5 blur-[150px] mix-blend-multiply dark:mix-blend-screen transition-all duration-1000" />
        <div
      ...
      <div className="relative z-10">
        ...
        {/* 1. HERO SERVICES (Densidad visual aumentada) */}
        <section className="relative px-6 pt-16 pb-20 text-center max-w-6xl mx-auto flex flex-col items-center">
          ...
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} className="hidden lg:flex absolute top-20 left-10 items-center gap-2 px-4 py-2 bg-white/40 dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm rotate-[-4deg]">
            <Cloud size={16} className="text-esoft-accent" /> <span className="text-sm font-bold text-gray-700 dark:text-gray-300">Cloud Native</span>
          </motion.div>
          <motion.div animate={{ y: [0, 10, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 1 }} className="hidden lg:flex absolute top-32 right-10 items-center gap-2 px-4 py-2 bg-white/40 dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm rotate-[4deg]">
            <ShieldCheck size={16} className="text-esoft-accent" /> <span className="text-sm font-bold text-gray-700 dark:text-gray-300">Enterprise Security</span>
          </motion.div>
          ...
          <motion.p
            ...
            className="text-lg md:text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto leading-relaxed font-light mb-12"
          >
            {t('servicesPage.hero.subtitle')}
          </motion.p>
        </section>

        {/* 2. LISTA DETALLADA DE SERVICIOS (Diseño sin vacíos) */}
          ...
          {services.map((service, index) => (
            ...
            >
              ...
              <div className="w-full lg:w-1/2 relative group">
                ...
                <div className="relative aspect-[4/3] md:aspect-video lg:aspect-square rounded-[2.5rem] overflow-hidden border border-gray-200/50 dark:border-white/10 shadow-2xl dark:shadow-[0_20px_50px_-15px_rgba(27,159,136,0.15)] group-hover:border-esoft-accent/30 transition-all duration-500">
                ...
                {/* NUEVO: Widget Flotante superpuesto para romper el rectángulo y llenar espacio */}
                <motion.div
                  initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }}
                  className={`absolute -bottom-6 ${index % 2 === 1 ? '-left-6' : '-right-6'} z-30 bg-white/90 dark:bg-[#0a110e]/90 backdrop-blur-2xl border border-gray-200 dark:border-white/10 p-5 rounded-2xl shadow-2xl flex items-center gap-4 hidden md:flex`}
                >
                  <div className="p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-100 dark:border-white/5">
                    {service.widgetIcon}
                  </div>
                  <div>
                    <h5 className="text-sm font-bold text-gray-900 dark:text-white leading-tight">{service.widgetTitle}</h5>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{service.widgetDesc}</p>
                  </div>
                </motion.div>
              </div>
              ...
              <div className="w-full lg:w-1/2 flex flex-col justify-center">
                ...
                <h2 className="text-4xl lg:text-5xl font-heading font-extrabold text-gray-900 dark:text-white tracking-tight mb-6">
                  {service.title}
                </h2>
                ...
                {/* Grid de Píldoras de Capacidades */}
                <div className="bg-white/40 dark:bg-white/[0.02] backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-gray-200/50 dark:border-white/5 shadow-sm mb-8">
                  <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-[0.15em] mb-6 flex items-center gap-2">
                    <Layers size={16} className="text-esoft-accent" /> {t('servicesPage.capabilitiesTitle', 'Capacidades Clave')}
                  </h4>
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {service.features.map((feature, i) => (
                      <li key={i} className="flex items-start gap-3 bg-white/60 dark:bg-white/5 p-3 rounded-xl border border-gray-100 dark:border-white/5 group/feature transition-colors hover:border-esoft-accent/30">
                        ...
                        <span className="text-sm text-gray-700 dark:text-gray-300 font-medium leading-tight">{feature}</span>
                      </li>
                    ))}
                  </ul>
                </div>

                {/* NUEVO: Fila de "Tech Stack" para anclar visualmente la parte inferior */}
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pt-4 border-t border-gray-200 dark:border-white/10">
                  <div className="flex flex-wrap gap-2">
                    {service.techTags.map((tag, i) => (
                      <span key={i} className="px-3 py-1 bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 text-xs font-medium rounded-lg border border-gray-200 dark:border-white/5">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <Link
                    to="/contacto"
                    className="inline-flex items-center gap-2 text-gray-900 dark:text-white font-bold pb-1 group whitespace-nowrap"
                  >
        ...
        <section className="w-full bg-white/60 dark:bg-[#0a110e]/80 backdrop-blur-2xl border-y border-gray-200/80 dark:border-white/10 py-12 overflow-hidden relative flex flex-col z-10 transition-all duration-500 shadow-xl mb-32">
          ...
          <div className="max-w-5xl mx-auto relative group">
            ...
            <div className="relative bg-gradient-to-br from-gray-900 to-black dark:from-[#0a110e] dark:to-[#050d0a] border border-gray-800 dark:border-white/5 rounded-[3rem] p-12 md:p-20 overflow-hidden shadow-2xl">
              ...
              <div className="relative z-10 flex flex-col items-center">
                <h3 className="text-3xl md:text-5xl font-heading font-extrabold mb-8 text-white tracking-tight">
                  {t('servicesPage.cta.title')}
                </h3>
                <p className="mb-12 text-lg text-gray-400 font-light leading-relaxed max-w-2xl mx-auto">
                  {t('servicesPage.cta.desc')}
                </p>
                <Link
                  to="/contacto"
                  className="px-10 py-4 bg-esoft-accent text-white font-bold rounded-full hover:bg-emerald-600 transition-all duration-300 shadow-[0_10px_30px_-10px_rgba(27,159,136,0.6)] hover:shadow-[0_15px_40px_-10px_rgba(27,159,136,0.8)] inline-flex items-center gap-3 hover:-translate-y-1"
                >
